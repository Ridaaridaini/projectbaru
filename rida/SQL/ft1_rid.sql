...
select 
	...
	coalesce(count(date_part('day',age(lr.end_date,lr.start_date))),0) as hari
from biodata b
join employee e
...
--3 Tampilkan fullname, jabatan, usia, dan jumlah anak dari masing-masing karyawan 
--saat ini (kalau tidak ada anak tulis 0 (nol) atau "-" saja),
Select
	b.first_name||''||b.last_name as fullname,
	p.name as jabatan,
	date_part('year',age(now(),dob)) as usia
...
select
	count(e.status='Kontrak') as jmlkontrak,
	count(e.status='Permanen') as jmlpermanen
from employee e
join biodata b
	on e.biodata_id=b.id

--6.Tampilkan nama karyawan, jenis perjalanan dinas, tanggal perjalanan dinas, 
--dan total pengeluarannya selama perjalanan dinas tersebut,
select
	b.first_name||''||b.last_name as fulname,
	tt.name as jenis,
	tr.start_date,
	tr.end_date,
...
group by fulname, jenis, tr.start_date,
	tr.end_date;
	
--8.Buatklah query untuk menampilkan nama lengkap karyawan, status pernikahan, 
--nama pasangan(suami/istri) yang sudah memiliki anak
select 
	b.first_name||''||b.last_name as fullname,
	b.marital_status
from family f
...
--Tampilkan data biodata yang tidak memiliki posisi kepegawaian
select 
	b.*
from biodata b
