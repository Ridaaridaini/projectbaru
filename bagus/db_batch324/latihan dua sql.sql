...
--Fungsi buat gabungin 2 atau lebih text

select first_name, last_name, concat(last_name,' ', last_name) as fullname from biodata
...
extract(year from to_date('18-08-2020', 'DD-MM-YYYY'))

-- fungsi to_chart = untuk konversi ke chart

-- Fungsi to date()
select '10-08-2023', to_date('10-08-2023', 'DD-MM-YYYY'), date_part('year', to_date('10-08-2023', 'DD-MM-YYYY'))


--- LATIHAN DATE PART

select dob, to_date(dob,'YYYY-MM-DD'), date_part('year',to_date(dob,'YYYY-MM-DD')),
to_char(now(), 'DD-MM-YYYY') from biodata
...
-- Tampilkan fullname, jabatan, usia dan jumlah anak dari masing masing karyawan


select concat(biodata.first_name,' ',biodata.last_name) as "Nama Lengkap", position.name as "Jabatan",
...
from biodata
inner join employee
on biodata.id = employee.biodata_id inner join employee_position
on employee.id = employee_position.employee_id inner join position
on employee_position.position_id = position.id left join family
on biodata.id = family.biodata_id
group by "Nama Lengkap", "Jabatan", "Usia"
...
select status,
	(case
	when status ilike 'Suami' then 0
	when status ilike 'Istri' then 0
	else 1
	end)
from family
group by status
...
-- hitung berapa hari cuti yang sudah di ambil masing masing karyawan
...
select concat(first_name,' ', last_name) as "Full Name",coalesce(sum (date_part('day',(age (end_date, start_date)))),0) as "Jatah Cuti"
...
select concat(first_name, ' ', last_name) as "Nama Lengkap", position.name as "Posisi", 
date_part('year',age(now(), to_date(dob, 'YYYY-MM-DD')))  as "Usia" from biodata
inner join employee on biodata.id = employee.biodata_id inner join employee_position on employee.id = employee_position.employee_id
inner join position on employee_position.position_id = position.id
order by "Usia" desc
limit 3

-- tampilkan nama nama pelamar yang tidak di terima karyawan
select concat(first_name,' ', last_name) as "Nama Lengkap",employee.id from biodata left join employee 
on biodata.id = employee.biodata_id
where employee.id is null

-- hitung berapa rata2 gaji karyawan dengan level staff
select avg(employee.salary) as "Gaji", position.name as "Jabatan"
