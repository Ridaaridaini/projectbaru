...
--fungsi concat untuk menggabungkan 2 atau lebih teks
SELECT
	first_name,
	last_name,
...
--fungsi now() : untuk mengambil waktu sekarang
SELECT now();

--fung date_part() : mengambil/extract dari bagian waktu
SELECT date_part('year', now());
select date_part('month', now());


SELECT date_part('year', '10-08-2023'); ---tidak bisa karena string
...
SELECT
	...
	date_part('year', to_date('10-08-2023', 'DD-MM-YYYY')),--DOUBEL
...
select
	dob, --TEXT
	to_date(dob, 'YYYY-MM-DD'),--DATE
	date_part('year', to_date(dob, 'YYYY-MM-DD')),--DOUBEL
	extract(year from to_date(dob, 'YYYY-MM-DD')),--NUMERIC
	to_char(to_date(dob, 'YYYY-MM-DD'), 'YYYY-MM-DD')--TEXT
FROM biodata;
...
SELECT
	...
	age(now(), to_date('01-10-2000', 'dd-mm-yyyy')),
	date_part('year', age(now(), to_date('01-10-2000', 'dd-mm-yyyy'))) || ' tahun '
